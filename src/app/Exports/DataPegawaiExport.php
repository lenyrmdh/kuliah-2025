    ...
    WithMapping
{
    ...
    public function map($pegawai): array
    {
        ...
        // Hitung total gaji sesuai 6 bulan terakhir dinamis (Jan–Jul 2025, atau sesuai now)
        $totalGaji6Bulan = $pegawai->gajis->filter(function ($gaji) {
            $tanggal = Carbon::parse($gaji->periode);
            return $tanggal >= now()->copy()->subMonths(5)->startOfMonth();
        })->sum('total_gaji');
        ...
        return array_merge([
            ...
            number_format($totalGaji6Bulan, 0, ',', '.'),
            $totalLemburJam,
        ]);
    }
